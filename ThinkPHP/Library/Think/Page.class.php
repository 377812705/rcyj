...
class Page{
    ...
    public function pageshow() {
    	...
    	$now_cool_page      = $this->rollPage/2;
    	$now_cool_page_ceil = ceil($now_cool_page);
    	$this->lastSuffix && $this->config['last'] = '尾页';
    	$this->lastSuffix && $this->config['first'] = '首页';
    	...
    	for($i = 1; $i <= $this->rollPage; $i++){
    		...
    		if($page > 0 && $page != $this->nowPage){
    			...
    			if($page > 0 && $this->totalPages != 1){
    				$link_page .= '<li class="lion"><a href="javascript:;">' . $page . '</a></li>';
    			}
    		}
    	}
    	...
    	$page_str = str_replace(
    			...
    			array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
    			$this->config['theme']);
    	return "<div  class='page'><ul>{$page_str}</ul></div>";
    	//<li class='goto'>            <input name='p' type='text'><li ><a href=''>确认</a></li>          </li>
    }  
